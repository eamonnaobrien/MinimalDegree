...
// Subgroup H is listed in the list Q.
// d(Z(Q[i])) = 2 for each i.
// Lists T1 and T2 each store a subgroup of H.

// Phi6, G with centre of type Cp x Cp x Cp

Table2Phi6 := function(p)
...
for p in PrimesInInterval (5, 11) do 
   ...
   H, T1, T2, MinDeg, GMinDeg := CheckPhi6Col3(L, p);
